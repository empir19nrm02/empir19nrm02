...
def display_responsivity( name, detectors, cieobs='1931_2', s_target_index=2, out_dir = None):
    if cieobs=='VS':
        cieobs='1951_20_scotopic'

    print( name)
    dl = lx.getwld(detectors[0])
    # LED_L41 is only availible in a modified version of luxpy at the moment
    ...
    for i in range(1, detectorNorm.shape[0]-1):
    ...
    ax1.plot(detectorNorm[0], detectorNorm[1], 'g-', label= r'target')
